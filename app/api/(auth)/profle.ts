...
export type CancheUserBodyPost =
  | {
      user: User;
    }
  | {
...
export async function POST(
  request: NextRequest,
): Promise<NextResponse<CancheUserBodyPost>> {
  ...
  let user = await getUserByUsername(result.data.username);
  ...
  const newUser = await updateUser(user, userData);
